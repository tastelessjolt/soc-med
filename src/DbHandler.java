...
public class DbHandler {
	// connection strings
	private static String connString = "jdbc:postgresql://localhost:9399/postgres";
...
public static JSONArray userFollow(String id){
	...
	public static JSONArray seePosts(String id, int offset, int limit){
		...
		try (
		    Connection conn = DriverManager.getConnection(
		    		connString, userName, "");
			PreparedStatement postSt = conn.prepareStatement("select postid,timestamp,uid,text,name,imageid from (select postid,timestamp,uid,text,imageid from post where post.uid in (select uid2 from follows where uid1 = ? UNION select uid from \"user\" where uid=? ) order by timestamp asc offset ? limit ?) as S natural left outer join \"user\"");
		)
		{	
			postSt.setString(1, id);
